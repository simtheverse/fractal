...
- [x] **5J2.1** Implement `NetworkMessage` subtrait (or equivalent) adding `Serialize + Deserialize` bounds without modifying the base `Message` trait
- [x] **5J2.2** Update `NetworkBus` to serialize/deserialize messages to bytes instead of cloning
- [x] **5J2.3** Test: messages round-trip through serialization with identical content
- [x] **5J2.4** Test: `NetworkBus` with serialization produces identical final state to `InProcessBus` (non-vacuous verification)
- [x] **5J2.5** All network serialization tests pass

### Track O: System Test Harness

**Requirements covered:** FPA-033, FPA-034, FPA-038

- [x] **5O.1** Write failing tests:
...
- [x] **5O.2** Implement system test harness:
...
- [x] **5O.3** Implement bottom-up regeneration:
...
- [x] **5O.4** All system test infrastructure tests pass
